...
void so_finalize(so_module *mod) {
  ...
  for (int i = 0; i < mod->phnum; i++) {
    ...
    if (p->p_type != PT_LOAD || (p->p_flags & PF_X) != PF_X)
      continue;
    uintptr_t seg_start = ((uintptr_t)mod->load_virtbase + p->p_vaddr) & ~0xFFFull;
    uintptr_t seg_end = ALIGN_MEM((uintptr_t)mod->load_virtbase + p->p_vaddr + p->p_memsz, 0x1000);
    if (mprotect((void *)seg_start, seg_end - seg_start, PROT_READ | PROT_EXEC) != 0)
      fatal_error("Error: mprotect RX failed for %s at %p", mod->name, (void *)seg_start);
  }
}
...
int so_load(so_module *mod, const char *filename) {
  ...
  tracePrintf("%s: load base = %p, size = %u KB\n", mod->name, mod->load_virtbase,
            ...
            if (sym->st_shndx == SHN_UNDEF) {
              ...
              if (addr) {
                *ptr = addr + rels[j].r_addend;
                ...
                missing++;
                tracePrintf("%s: unresolved import: %s\n", mod->name, name);
