...
static int mouse_filter_event(SDL_Event_min *e) {
  ...
  if (g_mouse_window &&
    ...
    if (e->type == SDLM_JOYBUTTONDOWN) {
      ...
      const int ok = engine_run_command("gameui_activate");
      tracePrintf("Start -> gameui_activate ret=%d\n", ok);
      if (!ok) {
...
XF(SDL_CreateSystemCursor) XF(SDL_DestroyWindow)
XF(SDL_FreeSurface) XF(SDL_GL_DeleteContext)
XF(SDL_GL_MakeCurrent)
...
XF(SDL_HasClipboardText) XF(SDL_HideWindow) XF(SDL_Init) XF(SDL_InitSubSystem)
XF(SDL_IsGameController) XF(SDL_JoystickClose) XF(SDL_JoystickGetDeviceGUID)
XF(SDL_JoystickGetGUIDString) XF(SDL_JoystickInstanceID)
XF(SDL_JoystickNameForIndex) XF(SDL_JoystickOpen) XF(SDL_LoadBMP_RW)
XF(SDL_NumJoysticks) XF(SDL_OpenAudioDevice) XF(SDL_OpenURL)
XF(SDL_PauseAudioDevice)
XF(SDL_QuitSubSystem) XF(SDL_RWFromFile) XF(SDL_RaiseWindow)
